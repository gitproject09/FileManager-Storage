package com.sopan.file_manager.activity;
...
import com.sopan.file_manager.adapter.FilesAdapter;
import com.sopan.file_manager.dialogs.AddItemsDialog;
import com.sopan.file_manager.dialogs.NewFolderDialog;
import com.sopan.file_manager.dialogs.NewTextFileDialog;
import com.sopan.file_manager.provider.Utils;
import com.sopan.file_manager.storage.EncryptConfiguration;
import com.sopan.file_manager.storage.Storage;
import com.sopan.file_manager.R;
import com.sopan.file_manager.dialogs.ConfirmDeleteDialog;
import com.sopan.file_manager.dialogs.RenameDialog;
import com.sopan.file_manager.dialogs.UpdateItemDialog;
import com.sopan.file_manager.storage.helpers.OrderType;
import com.sopan.file_manager.storage.helpers.SizeUnit;
...
public class MainActivity extends AppCompatActivity implements
                ...
                if (mMovingPath != null) {
                    ...
                    if (!mCopy) {
                        ...
                        if (!mMovingPath.equals(toPath)) {
                            ...
                            Utils.showSnackbar("Moved", mRecyclerView);
                            showFiles(getCurrentPath());
                        } else {
                            Utils.showSnackbar("The file is already here", mRecyclerView);
                        }
                    } else {
                        String toPath = getCurrentPath() + File.separator + "copy " + mStorage.getFile(mMovingPath)
                                .getName();
                        mStorage.copy(mMovingPath, toPath);
                        Utils.showSnackbar("Copied", mRecyclerView);
                        showFiles(getCurrentPath());
                    }
                    mMovingPath = null;
                }
    ...
    public void onClick(File file) {
        if (file.isDirectory()) {
            ...
            try {
                ...
                String mimeType =  MimeTypeMap.getSingleton().getMimeTypeFromExtension(Utils.fileExt(file.getAbsolutePath()));
                ...
                if (mStorage.getSize(file, SizeUnit.KB) > 500) {
                    Utils.showSnackbar("The file is too big for preview", mRecyclerView);
                    return;
                }
    ...
    private String getPreviousPath() {
        ...
        if (lastIndexOf < 0) {
            Utils.showSnackbar("Can't go anymore", mRecyclerView);
    ...
    public void onNewFolder(String name) {
        ...
        if (created) {
            ...
            Utils.showSnackbar("New folder created: " + name, mRecyclerView);
        } else {
            Utils.showSnackbar("Failed create folder: " + name, mRecyclerView);
        }
    }
    ...
    public void onNewFile(String name, String content, boolean encrypted) {
        ...
        showFiles(currentPath);
        Utils.showSnackbar("New file created: " + name, mRecyclerView);
    }
    ...
    public void onConfirmDelete(String path) {
        if (mStorage.getFile(path).isDirectory()) {
            ...
            Utils.showSnackbar("Folder was deleted", mRecyclerView);
        } else {
            mStorage.deleteFile(path);
            Utils.showSnackbar("File was deleted", mRecyclerView);
        }
        showFiles(getCurrentPath());
    }
    ...
    public void onRename(String fromPath, String toPath) {
        ...
        showFiles(getCurrentPath());
        Utils.showSnackbar("Renamed", mRecyclerView);
    }
